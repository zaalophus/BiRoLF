...
if lam_c_main is None:
    lam_c_main = 0.01  # bilinear-friendly default; RoLFLasso can override via CLI
    ...
    if agent_type == "linucb":
        ...
        if cfg.explore:
            agent = RoLFLasso(
                ...
                optimizer_max_iter = getattr(cfg, "optimizer_max_iter", 100)
            )
        else:
            agent = RoLFLasso(
                d=total_obs_dim,
        ...
        if cfg.explore:
            agent = BiRoLFLasso_Blockwise(
                ...
                lam_c_impute=cfg.lamc_bi_impute,
                lam_c_main=cfg.lamc_bi_main,
                block_oo_max_iter=getattr(cfg, "optimizer_max_iter", 100),
                block_ou_max_iter=getattr(cfg, "optimizer_max_iter", 100),
                block_uo_max_iter=getattr(cfg, "optimizer_max_iter", 100),
...
def plot_optimization_timing_comparison():
    ...
    fname_params = f"Case_{cfg.case}_M_{cfg.arm_x}_N_{cfg.arm_y}_xstar_{cfg.true_dim_x}_ystar_{cfg.true_dim_y}_dx_{cfg.dim_x}_dy_{cfg.dim_y}_T_{cfg.horizon}_explored_{cfg.init_explore}_noise_{cfg.reward_std}_trials_{cfg.trials}_seed_{cfg.seed}_run_{RUN_TAG}"
...
def run_main(given_cfg = None):
    ...
    RESULT_PATH = f"{MOTHER_PATH}/results/{date}/seed_{cfg.seed}_p_{cfg.p}_std_{cfg.reward_std}_run_{RUN_TAG}"
    FIGURE_PATH = f"{MOTHER_PATH}/figures/{date}/seed_{cfg.seed}_p_{cfg.p}_std_{cfg.reward_std}_run_{RUN_TAG}"
    LOG_PATH = (
        f"{MOTHER_PATH}/logs/{date}/seed_{cfg.seed}_p_{cfg.p}_std_{cfg.reward_std}_run_{RUN_TAG}"
    )
    ##
